...
function deleteDeclaration(period) {
...
function createGrossIncome({
  ...
  cy.get('body').then(($body) => {
    if ($body.find(`[data-test-id='delete-gross-income-button-${grossIncome.period}']`).length && override) {
      deleteDeclaration(grossIncome.period)
    }
...
function deleteInvoice({
  ...
  cy.get(`[data-test-id='delete-gross-income-button'][data-test-months*="${period}"]`).click()
...
function createInvoice({
  ...
  cy.get('body').then(($body) => {
    ...
    if (deleteAfter) {
      grossIncomes.forEach( g => {
        if ($body.find(`[data-test-months*="${g.period}"]`).length) {
          deleteInvoice({period: g.period})
        } 
      })
    }
...
describe('Settlement printable page', () => {
  ...
  it.skip('tax collector should be able to register 2 gross incomes', () => {
  ...
  it('tax collector should be able to create an invoice', () => {
    ...
    let monthsTooltip = '2024-10,2024-11'
